...
export default function AppointmentCard({ appointment }) {
  ...
  return (
    <div className="relative flex flex-col p-4 transition-shadow bg-white border border-gray-300 rounded-lg hover:shadow-md hover:border-primary">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <CircleCheckBig size={15} className="text-green-600" />
          <p className="text-sm text-green-600">
            {capitalizeWords(appointment.appointment_mode)} Appointment
          </p>
        </div>
        <div className="flex items-center gap-2">
      ...
      <div className="absolute flex items-center justify-between mt-2 bottom-3 right-3">
        ...
        {user.role === "patient" && appointment.status === "pending" && (
          <div className="flex items-center justify-end">
            <DeleteItem
              deleteUrl={`appointments/${appointment.id}/`}

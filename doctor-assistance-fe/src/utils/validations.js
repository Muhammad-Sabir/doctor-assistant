const validationRules = {
  ...
  phoneNo: {
    ...
    message:
      "Phone number must be in the format '+923039916210' or '03039916210'.",
  },
  confirmPassword: {
    ...
    test: (value) => {
      if (!isDateSelected(value)) {
        validationRules.dependentbirthDate.message =
          "Please select dependent date of birth.";
        return false;
      }
    ...
    test: (value) => {
      if (!isDateSelected(value)) {
        validationRules.experience.message =
          "Please select date of experience.";
        return false;
      }
    ...
    test: (value) => {
      if (!isDateSelected(value)) {
        validationRules.overrideDate.message =
          "Please select date you need to override schedule for.";
        return false;
      }
...
const isDateValid = (value, minYears) => {
  ...
  return (
    diff > minYears ||
    (diff === minYears && new Date().getMonth() >= new Date(value).getMonth())
  );
};
